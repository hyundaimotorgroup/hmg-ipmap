...
- **Docker** — for running local dependencies (PostgreSQL, Valkey)
- **Git**
...
Start PostgreSQL 17 and Valkey (Redis-compatible cache) via Docker Compose:
...
Application config for local development is in:

```
hmg-ipmap-api/src/main/resources/application-local.properties
```

Run the application with `--spring.profiles.active=local`.

### Install Git Hooks

Run once per clone to install the pre-commit hook (enforces commit message format):
...
```bash
./gradlew test --tests "com.hmg.ipmap.SomeTestClass"
```

### Code Formatting

This project enforces Google Java Format (AOSP style) via Spotless. CI will fail if formatting is inconsistent.
...
- Use [GitHub Issues](https://github.com/hyundaimotorgroup/hmg-ipmap/issues)
